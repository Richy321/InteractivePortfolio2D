...
Take the `*.workers.dev` hostname it prints and set `NET_CONFIG.url` in `js/net.js` to
`wss://<that-hostname>/ws`. The site itself deploys exactly as it did before - the
worker is a separate, independent deploy, and a `wss://` connection is allowed from
the `http://` site (it is not mixed content).
